...
********************************************************************************/
// void print_custom_bitmap(POINT Xstart, POINT Ystart, POINT Xend, POINT Yend, const unsigned char *pMap)
// void print_custom_bitmap(POINT Xstart, POINT Ystart, POINT width, POINT height, const uint8_t *myHeapArray)
void print_custom_bitmap(POINT Xpoint, POINT Ypoint, const unsigned char *pMap,
                   POINT Width, POINT Height)
{
  OLED_SetWindow(Xpoint, Ypoint, Xpoint+Width, Ypoint+Height);

  POINT i, j, byteWidth = (Width + 7) / 8; // make into chunks of 8 (bytes)
  ...
  for (j = 0; j < Height; j++) {
    for (i = 0; i < Width; i++) {
      ...
      // Serial.print(j*8 + i);
      // Serial.print(", ");
      // Serial.print(pgm_read_byte(pMap + j * byteWidth + i / 8) & (128 >> (i & 7)));
      if (pgm_read_byte(pMap + j * byteWidth + i / 8) & (128 >> (i & 7))) { // goes through every byte and every iteration reads each bit from left to right. If it's 1, itll, draw, otherwise no. 
        // Serial.print(", on");
        OLED_WriteData(0x77);
      } 
      else
                   ...
                   POINT Width, POINT Height)
{
  ...
  for (j = 0; j < Height; j++) {
    for (i = 0; i < Width; i ++) {
      if (pgm_read_byte(pMap + j * byteWidth + i / 8) & (128 >> (i & 7))) { // goes through every byte and every iteration reads each bit from left to right. If it's 1, itll, draw, otherwise no. 
        // IS BROKEN  TO USE THIS FIGURE IT OUT~~~~
